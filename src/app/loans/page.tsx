...
export default function LoansPage() {
  ...
  const spentThisCycleByCategory = useMemo(() => {
    if (!loans || rawTransactions.length === 0) return {} as Record<string, number>;
    const result: Record<string, number> = {};
    for (const loan of loans) {
      if (!loan.payoffDate) continue;
      const dn = loan.description ? `${loan.category} - ${loan.description}` : loan.category;
      result[dn] = getSpentThisCycle(rawTransactions, dn, loan.payoffDate, loan.paymentFrequency as any);
    }
    return result;
  }, [loans, rawTransactions]);
    ...
    try {
      ...
      if (loan) {
        ...
        const { available } = getAvailable({
          amount: loan.paymentAmount || loan.totalBalance || 0,
          frequency: loan.paymentFrequency as any,
          dueDate: loan.payoffDate || null,
          startDay: wsOn,
  ...
  const { weeklyTotal, overBudgetTotal } = useMemo(() => {
    ...
    for (const loan of loans) {
      const dn = loan.description ? `${loan.category} - ${loan.description}` : loan.category;
      const { available: avail, weeklyRate: wkAmt } = getAvailable({
        amount: loan.paymentAmount || loan.totalBalance || 0,
        frequency: loan.paymentFrequency as any,
        dueDate: loan.payoffDate || null,
        startDay: wsOn,
      ...
      weekly += wkAmt;
      if (avail < 0) overBudget += Math.abs(avail);
    }

    return { weeklyTotal: weekly, overBudgetTotal: overBudget };
  }, [loans, allTimeSpentByCategory, budgetStartDateByCategory, userProfile, spentThisCycleByCategory]);
