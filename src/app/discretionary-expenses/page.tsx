...
export default function DiscretionaryExpensesPage() {
  ...
  const loadWeeklyTransactions = useCallback(async () => {
    ...
    try {
      ...
      snapshot.forEach((d) => {
        const data = d.data();
        if (!data.date || data.autoGenerated) return;
      ...
      snapshot.forEach((d) => {
        const data = d.data();
        if (data.date && !data.autoGenerated) {
            ...
            <div className="divide-y">
              {loading ? (
                ...
                sortedExpenses.map((expense) => {
                  ...
                  return (
                    <div key={expense.id} className="px-4 py-3 hover:bg-muted/50 transition-colors">
                      ...
                      {expense.dueDate && (
                        <p className="text-[11px] text-muted-foreground mt-1 ml-11">
                          Due {format(parseISO(expense.dueDate), 'MMM d')} — saving {formatCurrency(weeklyAmount)}/wk to reach {formatCurrency(expense.plannedAmount)} on time
                        </p>
                      )}
                    </div>
                  );
